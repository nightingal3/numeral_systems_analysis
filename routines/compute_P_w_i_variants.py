...
def compute_P_w_i(nnum, ncat, numberline, mus, c, w):
    ...
    P_w_i_vec = np.divide(1, math.sqrt(2 * math.pi) * w * Means) * \
        np.exp(np.divide(-np.square(Js-Means), 2 * np.square(w * Means)))
...
def compute_f_i_w_numerator(nnum, ncat, numberline, mus, cc, w):
    ...
    f_i_w_vec = np.divide(1, math.sqrt(2*math.pi) * w * Means) * \
        np.exp(-(Js - Means) ** 2 / (2 * (w * Means) ** 2))
...
if __name__ == "__main__":
    ...
    print(compute_f_i_w_numerator(100, 2, [i for i in range(
        1, 101)], np.asarray([18, 18]).transpose(), 2.2810, 0.31))
