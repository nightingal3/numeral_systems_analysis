...
def compute_approx_cost(term, numberline, num_term_pt, end_category, nd, mu_range=range(20), w=0.31):
    ...
    term_num_map, nterm = get_term_num_matrix(
        term, nnum, num_term_pt, end_category, numberline)
    a = np.amax(term_num_map, axis=0)
    mmap = [x+1 for x in np.argmax(term_num_map, axis=0)]

    (mus, P_w_i_vec) = compute_P_w_i_match_modemap(
        mmap, numberline, nterm, term_num_map, mu_range, cc, w, nd)
    mus = [i+1 for i in mus]
    F_i_w_numerator = compute_f_i_w_numerator(
        nnum, nterm, numberline, mus, cc, w)
...
def compute_cost_size_principle(upper_lim, need_prob):
    """ Compute approximate cost through the size principle, where cost = 1/category size.
    Bayesian listener has been added, so that for each term the normalized need probability for that number given the term is taken into account.
    Assumes one term for every number, up to some point above which there's a general "many" term
    """
    ...
    for i in range(upper_lim, length):
        unit_cost[i] = -math.log(float(need_prob[i]) /
                                 float(denom)/float(denom_normalize), 2)
...
def compute_cost_size_principle_arb(modemap, need_prob):
    """ Generalized version of the simple compute_cost_size_principle function, doesn't assume that there's one term for lower numbers
    """
    l = len(need_prob)
    unit_cost = [0] * l
    ...
    for i in range(len(unique_cat)):
        inds = [j for j, val in enumerate(
            modemap) if modemap[j] == unique_cat[i]]
        denom_normalize = 0
        for ind in inds:
            denom_normalize += float(need_prob[ind])/float(len(inds))
        for ind in inds:
            unit_cost[ind] = -math.log(float(need_prob[ind]) /
                                       float(len(inds)/float(denom_normalize)), 2)
