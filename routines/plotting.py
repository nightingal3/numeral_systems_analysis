...
def plot_uid(attested_uid_dev, alternate_uid_dev, langname, attested_color="red", alternate_color="green"):
    ...
    plt.legend(prop={"size": font_sizes["MEDIUM_LARGE_SIZE"]}, loc="upper right")
    plt.title("Uniform information density (UID)", fontsize=font_sizes["BIG_SIZE"])
...
def plot_mini(info_trajs, langname, color1="blue", color2="orange", ext=".png", root_dir="uid/"):
    ...
    """
    alt = langname + "_alt"
    langname_proper = {"mand": "Mandarin","eng_1000": "English", "eng": "English", "fre": "French", "ger": "German", "ita": "Italian", "spa": "Spanish", "uni": "Template"}
    for num in info_trajs[langname]:
        ...
        plt.plot(numberline, info_trajs[langname][num], color=color1, label="base-atom", linewidth=2) 
        plt.plot(numberline, info_trajs[alt][num], color=color2, linestyle=":", label="atom-base", linewidth=2) 
    ...
    plt.plot(numberline, uid_traj, color="red", linestyle="--", label="UID", linewidth=2)
...
def plot_bounding_box(ax, bound_range, area_dict, color="#d8ac31", linewidth=3):
    ...
    ax.add_patch(Rectangle((0, bound_range[0]), (bound_range[1] - bound_range[0]), max_height_in_bounds, fill=None, color=color, linewidth=linewidth))
