...
def pearson_correlation_x_langs(filenames, out_file="cross_language_variation.png"):
        freq = {}
        for root, dirs, files in os.walk(filepath):
		for f in files:
			freq[d] = []
			with open(filepath + "/" + file, "r") as np_file:
                                np = np_file.read().split('\r\n')
                                freq[d].extend(np)
        print(freq)
	...
	for s_r, s_d, s_f in os.walk(filepath):
		for mapping_file in s_f:
			if mapping_file[:-4] in langs or langs == "all":
				...
				#f = codecs.open(filepath + "/" + mapping_file, "r")
				lines = f.read().split('\r\n')[:-1]
				df = pd.read_csv(filepath + "/" + mapping_file, encoding="utf-8")
				#df = pd.read_csv(filepath + "/" + mapping_file)
...
if __name__ == "__main__":
        ...
        #eng_num = [float(item) for sublist in list(eng_reader) for item in sublist]
        
        
        
        #dig = [float(item) for sublist in list(dig_reader) for item in sublist]
        #num = [float(item) for sublist in list(num_reader) for item in sublist]
        #pearson_correlation_num_dig(num, dig, "ita_pos_num_vs_dig.png", "Italian (POS tag)", upper_bound=100)
	...
	print(total_dict)
	#ld = collect_lang_data_words("downloads/google_ngrams", mapping, dict_name="english_num_freq.p", decades=[1980, 2000], langs=["english"])
	assert False
	#word_to_num_eng = mapping["english"]
	#num_freq = pickle.load(open("total_word_freq.p", "rb"))["english"]
	#ld = collect_lang_data_numerals("downloads/google_ngrams")
	#ld = collect_lang_data_words("downloads/google_ngrams", mapping, dict_name="eng_mand_word_freq.p", decades=[1980, 2000])
	#word_freq = ld["english"]
	...
	#plot_numeral_vs_word(num_freq, word_freq, word_to_num_map, "english", 1, 100)	
